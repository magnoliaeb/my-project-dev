<template>
      <!-- col -->
  <div class="mb-4 full-width md:w-1/2 p-1 lg:w-1/3">
    <!-- video -->
    <article class="shadow  border-l-8 bg-white" :style="`border-color: ${tag.bg_color_hex}`">
      
      <div class="px-4 py-2" style="">
        <h2
          class="text-lg font-bold leading-tight md:text-2xl"
        >
        <nuxt-link :to="{ name: 'tags-tag', params: { tag: tag.name }}">#{{ tag.name }}</nuxt-link></h2>
        <div class="mt-2 md:flex justify-end">
            <div>
                <button
